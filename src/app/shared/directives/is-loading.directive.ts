...
export class IsLoadingDirective {
  ...
  public set appIsLoading(value: boolean) {
    if (!value) {
      const firstChild: HTMLElement = this._el.nativeElement.children[0];
      if (!firstChild || firstChild.tagName !== 'BTN-LOADING-CONTENT') {
        return;
      }

      const childrens: any = firstChild.childNodes;
  ...
  public loaderEl: HTMLDivElement = new HTMLDivElement;
